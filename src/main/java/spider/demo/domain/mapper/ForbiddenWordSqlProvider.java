...
public class ForbiddenWordSqlProvider {

    public String countByExample(ForbiddenWordExample example) {
        SQL sql = new SQL();
        sql.SELECT("count(*)").FROM("forbidden_word");
        applyWhere(sql, example, false);
        return sql.toString();
    }

    public String deleteByExample(ForbiddenWordExample example) {
        SQL sql = new SQL();
        sql.DELETE_FROM("forbidden_word");
        applyWhere(sql, example, false);
        return sql.toString();
    }
    ...
    public String selectByExample(ForbiddenWordExample example) {
        SQL sql = new SQL();
    ...
    protected void applyWhere(SQL sql, ForbiddenWordExample example, boolean includeExamplePhrase) {
        if (example == null) {
            return;
        }
        ...
        for (int i = 0; i < oredCriteria.size(); i++) {
            ForbiddenWordExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                for (int j = 0; j < criterions.size(); j++) {
                    ForbiddenWordExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
